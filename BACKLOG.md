...
- [x] Feedback visual: colores de acierto/error en el globo (verde/rojo/dorado) y overlays (sin iconos, tinte de pantalla)
  - A/B: verde en acierto, rojo→dorado en error con etiquetas blancas + flyTo con zoom adaptativo al país correcto
  - C-F: verde en acierto, dorado en error
- [x] Zoom adaptativo al tamaño del país en tipos C-F (efecto "viaje")
  - C/D: flyTo al centroide (no capital) para evitar que ChoicePanel tape el país
  - E/F: zoom máximo ampliado (×25) para microestados (San Marino, Mónaco)
  - C-F: marcadores de microestados ocultos (no se hace click en el mapa en estos tipos)
