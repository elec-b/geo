...
- [ ] **(i - añadido para añadir 6 idiomas)** Añadir 6 idiomas nuevos (el, ca, da, fi, sk, hr): ejecutar pipeline (e)-(h) — datos multi-idioma (CLDR + Wikidata + Claude), validación, UI traducida, Wikipedia slugs. Añadir a `SUPPORTED_LOCALES` y generar archivos de locale. Al generar los `game.json`, verificar categorías CLDR de cada idioma (`Intl.PluralRules(locale).resolvedOptions().pluralCategories`) y añadir todas las formas plurales necesarias en claves como `selector.countryCount` — sk y hr tienen formas complejas (sk: `one/few/many/other`; hr: `one/few/other`) y necesitarán las mismas claves `_few`/`_many` que se añadieron a ru/uk/pl/cs/ro. Aplicar esta verificación a cualquier idioma nuevo en el futuro.
- [ ] **(j - añadido para añadir 6 idiomas)** Mapeo de códigos de locale para publicación en stores. 19 de 26 códigos nuestros no coinciden con los de App Store / Google Play (ej. `nb` → `no`/`no-NO`, `zh-Hans` → `zh-CN`). Crear tabla de mapeo en config de publicación / fastlane. Ver spike para tabla completa
- [ ] **(k - añadido para añadir 6 idiomas)** Registrar variantes regionales en stores (coste cero): es-419/es-MX, fr-CA, en-GB, en-AU, zh-HK. Reutilizan traducciones base existentes, mejoran descubrimiento
