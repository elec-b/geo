...
- Feedback adicional del usuario (segunda ronda):
  ...
  - [ ] Tabla → Globo: al navegar desde la tabla al globo, no se muestran los toggles de etiquetas (Países / Capitales). Solo aparecen al pulsar manualmente el pill «Globo». Causa: la condición de render de los toggles usa `mode === 'countries'`, pero al venir de tabla `mode` sigue siendo `'capitals'` (con `capitalsGlobeView = true`)
