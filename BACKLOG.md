...
- [ ] **(fix)** en el selector de jugar, testeando en ruso (creo que era ruso), aparece el número de países de cada nivel en inglés, e.g. para europa-turista "10 countries", para europa-mochilero "27 countries", debería aparecer en ruso. Chequear para todos los países, no solo para ruso. Si necesitas screenshot, te puedo pasar uno.
...
- [ ] **(i)** Añadir 6 idiomas nuevos (el, ca, da, fi, sk, hr): ejecutar pipeline (e)-(h) — datos multi-idioma (CLDR + Wikidata + Claude), validación, UI traducida, Wikipedia slugs. Añadir a `SUPPORTED_LOCALES` y generar archivos de locale
- [ ] **(j)** Mapeo de códigos de locale para publicación en stores. 19 de 26 códigos nuestros no coinciden con los de App Store / Google Play (ej. `nb` → `no`/`no-NO`, `zh-Hans` → `zh-CN`). Crear tabla de mapeo en config de publicación / fastlane. Ver spike para tabla completa
- [ ] **(k)** Registrar variantes regionales en stores (coste cero): es-419/es-MX, fr-CA, en-GB, en-AU, zh-HK. Reutilizan traducciones base existentes, mejoran descubrimiento
